...
import { ArrowLeft, MapPin, Calendar, Info, Package, User, ShieldCheck, QrCode } from 'lucide-react';
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-12">
            <div className="lg:col-span-5 aspect-square lg:aspect-auto bg-zinc-50 relative">
              ...
              <div className="absolute bottom-8 left-8">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-xl backdrop-blur-md ${
                  item.condition === 'B' ? 'bg-emerald-500/90 text-white' :
                  item.condition === 'RR' ? 'bg-amber-500/90 text-white' :
                  'bg-red-500/90 text-white'
                }`}>
                  Kondisi: {item.condition === 'B' ? 'Baik' : 
            ...
            <div className="lg:col-span-7 p-10 lg:p-16 flex flex-col">
              <div className="mb-12">
                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.2em] mb-4">Detail Aset Elektronik (LHI)</p>
                <h1 className="text-4xl lg:text-5xl font-bold text-zinc-900 tracking-tight leading-[1.1]">{item.name}</h1>
                <p className="text-zinc-400 font-mono text-xs mt-4">{item.brand_type || 'Tanpa Merek'}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 mb-12">
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-zinc-300">
                    <MapPin size={14} />
                    <p className="text-[10px] font-bold uppercase tracking-widest">Ruang / Bagian</p>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-zinc-300">
                    <User size={14} />
                    <p className="text-[10px] font-bold uppercase tracking-widest">Pemakai</p>
                  </div>
                  <p className="text-zinc-900 font-semibold text-lg">{item.user_name || 'Belum ada'}</p>
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">{item.user_status} - {item.user_position}</p>
                </div>

                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-zinc-300">
                    <Calendar size={14} />
                    <p className="text-[10px] font-bold uppercase tracking-widest">Tahun Perolehan</p>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-zinc-300">
                    <QrCode size={14} />
                    <p className="text-[10px] font-bold uppercase tracking-widest">Kode Barang / Register</p>
                  </div>
                  <p className="text-zinc-900 font-semibold text-lg">{item.item_code} / {item.register_code}</p>
                  <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">NIBAR: {item.nibar || '-'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 mb-12">
                <div className="space-y-4 p-8 bg-zinc-50 rounded-[32px] border border-zinc-100/50">
                  <div className="flex items-center gap-2 text-zinc-400">
                    <Info size={16} />
                    <p className="text-[10px] font-bold uppercase tracking-widest">Spesifikasi Barang</p>
                  </div>
                ...
                {item.notes && (
                  <div className="space-y-4 p-8 bg-zinc-50 rounded-[32px] border border-zinc-100/50">
                    <div className="flex items-center gap-2 text-zinc-400">
                      <Info size={16} />
                      <p className="text-[10px] font-bold uppercase tracking-widest">Keterangan (Ket.)</p>
                    </div>
                    <p className="text-zinc-600 leading-relaxed text-sm font-medium whitespace-pre-wrap italic serif">{item.notes}</p>
                  </div>
                )}
              </div>
