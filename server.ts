...
// Migration: Add new columns to items
try { db.prepare("ALTER TABLE items ADD COLUMN nibar TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN register_code TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN item_code TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN brand_type TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN user_status TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN user_position TEXT").run(); } catch (e) {}
try { db.prepare("ALTER TABLE items ADD COLUMN notes TEXT").run(); } catch (e) {}
...
async function startServer() {
  ...
  app.post("/api/items", (req, res) => {
    ...
    const info = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(name, nibar, register_code, item_code, specifications, brand_type, procurement_year, user_name, user_status, user_position, location, condition, photo_url, notes);
  ...
  app.put("/api/items/:id", (req, res) => {
    ...
    db.prepare(`
      UPDATE items 
      SET name = ?, nibar = ?, register_code = ?, item_code = ?, specifications = ?, brand_type = ?, procurement_year = ?, user_name = ?, user_status = ?, user_position = ?, location = ?, condition = ?, photo_url = ?, notes = ?
      WHERE id = ?
    `).run(name, nibar, register_code, item_code, specifications, brand_type, procurement_year, user_name, user_status, user_position, location, condition, photo_url, notes, req.params.id);
  ...
  app.post("/api/requests", (req, res) => {
    const { item_name, user_name, department, request_date, reason, quantity } = req.body;
    const info = db.prepare(`
      INSERT INTO requests (item_name, user_name, department, request_date, reason, quantity)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(item_name, user_name, department, request_date, reason, quantity);
